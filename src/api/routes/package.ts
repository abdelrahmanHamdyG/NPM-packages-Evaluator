...
router.put('/:id', async (req: Request, res: Response): Promise<void> => {
  ...
  router.post('/', async (req: Request, res: Response): Promise<void> => {
    ...
    if ((!Content && !URL) || (Content && URL)) {
      res.status(400).json({
        error: 'There is missing field(s) in the PackageData, or both Content and URL are set, which is invalid.',
      });
      return;
    }
    ...
    try {
      if (URL) {
        ...
        await handleURLBasedUpload(req, res, authToken, URL, JSProgram);
      } else if (Content) {
        // Handle Content-based ingestion
        await handleContentBasedUpload(req, res, authToken, Content, JSProgram);
      }
    } catch (error) {
